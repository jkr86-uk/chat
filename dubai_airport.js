...
const entity_name='dubai_airport'
let user = {};
let chatSession = {};
let chatArray = [];
...
function createChatListItem(item) {
    ...
    const responsItem = `<div class='w-full max-w-[376px]'>
    ...
    const actionItem = `<div class='w-full max-w-[376px]'>
<div class='w-full flex items-end gap-x-2 justify-end'>
    <div class='px-4 py-2 bg-[#F2F3F6] rounded-t-[8px] rounded-bl-[8px] flex-1'>
    ${!item?.audioFile ? `<div class='text-[14px] text-[#495160] leading-[22px]'>
    <p>${item.content}</p>
</div>` : `<div class="">
...
const tx = document.getElementById("your-prompt");
// Set the initial height to 40px
tx.style.height = "51px";
tx.style.overflowY = "hidden";

tx.addEventListener("input", OnInput, false);

function OnInput() {
    this.style.height = "51px"; // Set the initial height to 40px
...
function renderMarkdown(markdownContent) {
    ...
    // Use the marked library to parse the Markdown content
